...
import org.techtown.omni_beta100.view.WAVHeader;
...
public class SoundFile {
    ...
    public static SoundFile create(String fileName,
        ...
        if (ObjectUtilss.isEmpty(fileName)) {
            Log.v("뭘봐", f.getName());
        } else {
            Log.v("등신", f.getName());
        }
        if (ObjectUtilss.isEmpty(f)) {
            Log.v("뭘봐s", "fffffff");
        } else {
            Log.v("등신s", "fffffff");
        }
...
//            return mDecodedSamples.asReadOnlyBuffer();
    ...
    private void readFiles(File inputFile) throws InvalidInputException, IOException {
        ...
        for (p = 0; p < numTracks; p++) {
            format = extractor.getTrackFormat(p);
            if (format.getString(MediaFormat.KEY_MIME).startsWith("audio/")) {
                extractor.selectTrack(p);
                break;
            }
        }
        if (p == numTracks) {
        ...
        while (true) {
            ...
            if (mDecodedSamples.remaining() < 1024) {
                Log.v("mDecodedSamplesremaining", mDecodedSamples.remaining() + " ");
    ...
    private void ReadFile(File inputFile)
        ...
        mFileSize = (int) mInputFile.length();
        extractor.setDataSource(mInputFile.getPath());
        //extractor.setDataSource("file://"+mInputFile.getPath());
        int numTracks = extractor.getTrackCount();
        // find and select the first audio track present in the file.
        ...
        // only once.
        mDecodedBytes = ByteBuffer.allocate(1 << 40);
        ...
        while (true) {
            ...
            if (!done_reading && inputBufferIndex >= 0) {
                ...
                Log.v("sample_size", sample_size+" ");
                Log.v("format.getString(MediaFormat.KEY_MIME)", format.getString(MediaFormat.KEY_MIME));
                if (firstSampleData
                ...
                if (mDecodedBytes.remaining() < info.size) {
                    ...
                    while (retry > 0) {
                        try {
                            ...
                            Log.e("SoundFile", oome.getMessage());
                            retry--;
                        }
                    }
                    if (retry == 0) {
